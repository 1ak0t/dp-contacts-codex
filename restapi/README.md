...
По умолчанию сервер слушает `0.0.0.0:4000` (все сетевые интерфейсы).
Адрес и порт задаются через `HOST` и `PORT` в `.env`.
После сборки фронтенда сервер также отдаёт интерфейс на `/`, API доступен
через `/api/auth`, `/api/contacts` и `/api/health`. Старые маршруты сохранены.
Инструкция размещения на `192.168.10.119` находится в [README проекта](../README.md).
